...
assets {
  ...
  minifyOptions = [
    ...
    // allowDynamicImport: true,     // Enable import() expression support (requires dynamicImportAlias)
    // dynamicImportAlias: '__import__' // Function name that import(x) calls are rewritten to
  ]
